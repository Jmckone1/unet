...
class Pre_process():
    ...
    def reformat_dataset(Old_path, New_path, folds_value = 10, saveFile = False, saveCSV = False, saveBilinear = False, resize_axis = 1, debug = False):
        ...
        for dir_name in tqdm(image_directories):
            ...
            image = nib.load(os.getcwd() + Old_path + "imagesTr/" + dir_name)
            label = nib.load(os.getcwd() + Old_path + "labelsTr/" + dir_name)
            ...
            for image_slice in tqdm(range(np.shape(numpy_image)[size-1])):
                ...
                if saveFile == True:
                    ...
                    nib.save(new_image, os.getcwd() + New_path + "imagesTr/" + dir_name[:-7] + "_" + str(image_slice) + dir_name[-7:])
                    nib.save(new_label, os.getcwd() + New_path + "labelsTr/" + dir_name[:-7] + "_" + str(image_slice) + dir_name[-7:])

                csv_append.append(dir_name[:-7] + "_" + str(image_slice) + dir_name[-7:])
        ...
        if saveCSV == True:
            Pre_process.create_csv(csv_append, fold, os.getcwd() + New_path + "Training_dataset.csv")
    ...
    def reformat_dataset_brats(Old_path, New_path, folds_value = 10, saveFile = False, saveCSV = False, saveBilinear = False, resize_axis = 1, debug = True, dir_search = False):
        ...
        for dir_name in tqdm(range(len(image_directory))):
            ...
            for image_slice in tqdm(range(np.shape(numpy_image)[size-1])):
                ...
                if saveFile == True:
                    ...
                    # save each slice as an image to the new folder that is created here.
                    nib.save(new_image, os.getcwd() + New_path + "imagesTr/" + image_directory[dir_name][:-7] + "_" + str(image_slice) + image_directory[dir_name][-7:])
                    nib.save(new_label, os.getcwd() + New_path + "labelsTr/" + label_directory[dir_name][:-7] + "_" + str(image_slice) + label_directory[dir_name][-7:])

                csv_append.append(image_directory[dir_name][:-7] + "_" + str(image_slice) + image_directory[dir_name][-7:])
        ...
        if saveBilinear == True:
            Pre_process.calc_Bi_Linear(New_path)
        return [csv_append, fold, os.getcwd() + New_path + "Training_dataset.csv", old_path_list]

    def create_csv(directory_names, fold, save_directory, old_path_list = []):
        print("Creating csv")
        ...
        indexes = range(len(directory_names))
        
        if old_path_list == []:
            old_path_list = np.zeros(np.size(indexes))
        
        stored_values = np.stack((directory_names, indexes, fold, old_path_list), -1)
...
#     New_path = "/brats_Dataset/"
    ...
    for input_path in Old_paths:
        data_out = Pre_process.reformat_dataset_brats(input_path,
                                                              ...
                                                              resize_axis = 1, 
                                                              debug = False)
        csv_data.extend(data_out[0])
        folds.extend(data_out[1])
        new_dir.extend(data_out[2])
        old_dir.extend(data_out[3])
    Pre_process.create_csv(csv_data,folds, os.getcwd() + New_path + "Training_dataset.csv" ,old_dir)
    Pre_process.calc_Bi_Linear(New_path)
