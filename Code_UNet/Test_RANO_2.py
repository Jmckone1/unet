...
def input_data(path ="Brats_2018_data_split/Validation", path_ext = ["/HGG","/LGG"]):
    ...
    d, f = [], []
    index_max = []
    index_max.extend([0]) # init index max at 0 so that values can be summed
    ...
    for input_ in range(len(path_ext)):
        ...
        for (dir_path, dir_names, file_names) in walk(path + path_ext[input_]):
            # gets rid of any pesky leftover .ipynb_checkpoints files (caused huge issue when running outputs in notebook)
            if not dir_names == []:
                if not dir_names[0].startswith("."):

                    # collate all directory names and update counter
                    d.extend(dir_names)
                    counter = len(d)
    
        # sets the start and end values of the file loop
