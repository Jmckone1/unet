...
from os import walk
from skimage.measure import regionprops, find_contours#, mesh_surface_area, marching_cubes
from skimage.morphology import label, binary_erosion, disk, binary_dilation#, binary_opening
...
if __name__ == "__main__":
    ...
    print(os.path.join(path,os.getcwd(),"/Task06_Lung/labelsTr/"))

    path = os.getcwd() + "/Task06_Lung/labelsTr/"
    c_s = 0
    ...
    overwrite = True
    RANO_out_path = "_RANO.npz"
#    segment_in_path = "_whseg_norm.nii.gz"
...
#    name = ["HGG/","LGG/"]
    ...
    for x in tqdm(range(output_size)):
        ...
        if overwrite == True:
            ...
            if os.path.exists(path[:-9] + "labelsRANO/" + d[x][:-7] + RANO_out_path):
                os.remove(path[:-9] + "labelsRANO/" + d[x][:-7] + RANO_out_path)
        if not os.path.exists(path[:-9] + "labelsRANO/" + d[x][:-7] + RANO_out_path):
            if not os.path.exists(path[:-9] + "labelsRANO/"):
                os.makedirs(path[:-9] + "labelsRANO/")
            data_Plot = nib.load(path + d[x])
            input_2 = data_Plot.get_fdata()

            Bi_output = calc_2D_RANO_measure(input_2, pixdim=(input_2.shape[0],input_2.shape[1]), affine=None, mask_value=0, axis=2, calc_multiple=False, background_image=None, output_filepath="output_rano", verbose=False)

            np.savez(path[:-9] + "labelsRANO/" + d[x][:-7] + RANO_out_path, RANO=Bi_output)